...
{    
	...
	ifstream infile1("C:\\Wasif\\find_NN\\genome_hash\\10k_signature_genome_3500.txt");
...
{
	string file_name;
   file_name="C:\\Wasif\\find_NN\\Lsh_bucket_genome\\3500_10k\\bucket_u";
	 char ch1,ch2,ch3;
       if(u<10)
      {
        ch1=u+'0';
  ...
  {   
	...
	//for(i=1+r*n_user;i<r*n_user+5;i++)  
	  for(i=u+r*10000;i<r*10000+u+1;i++)
	{  
		 cout<<"target user:"<<" "<<i<<endl;
		p=0;
	 // for(j=1+r*n_user;j<r*n_user+n_user;j++)
		for(j=1+r*10000;j<r*10000+10000;j++)
	  {
		  if(i==j)continue;
		  if(user1[i]==user1[j])
		  {
